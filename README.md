...
* COVID-19로 인해 강의실 사용을 예약제로 바꾸면서 Google Form을 통한 좌석 예약 및 입실/퇴실 명부 관리로 기존에 취약했던 COVID-19 감염 및 전파의 대비책이 잘 마련되었습니다.

반면, 바뀐 방법의 불편한 점은 다음과 같습니다.

* 이 시스템을 이용하면서 학생들이 예약, 입실, 퇴실마다 Google Form을 작성해야 하는 번거로움이 존재합니다.
* 또 이를 관리하는 사람들이 일일이 명단을 확인하고 좌석 배치까지 해야 합니다.
...
1. 현재 경북대학교 컴퓨터학부만 대상으로 하지만, 상용화된다면 실제 경북대학교 내의 학부별로 사용할 수 있는 빈 강의실과 좌석 등을 파악하여 경북대학교 학우 전체가 학업에 있어 스마트하고 편리한 캠퍼스 생활을 할 수 있도록 할 것입니다. 이를 응용하면 타 학교로도 확장 가능할 것으로 보입니다.

2. 교수님들에게도 원활한 홍보를 통해 편하게 이용하실 수 있도록 할 것입니다.
...
https://youtube.com/shorts/CQ8SKpUrV5w?feature=share

https://youtube.com/shorts/YqlICyIuu5E?feature=share

시연 영상 촬영을 위한 에뮬레이터 실행 중 문제가 발생하여 부득이하게 진행 과정 부분을 캡쳐하여 영상으로 편집하였습니다.
어플 실행과정의 이해도를 돕기 위하여 아래에 적힌 간단한 설명을 참고해주시면 감사하겠습니다.

1. 교수/학생/교직원용으로 본인의 신분을 나타낸 뒤 회원가입 및 로그인 진행

2. 로그인 후 IT4호관과 융복합관 중 본인의 상황에 맞는 건물 선택

3. 강의실 대여 및 좌석 예약을 위한 날짜 선택

4. 교수님 페이지
 4-1. 강의실 선택 후 다음의 정보를 기입
   1-1. 시간은 1시간 단위로 스크롤바를 내려 선택할 수 있습니다.

 4-2. 정보가 제대로 기입되면 관리자에게 승인 요청이 전달되며 관리자 승인 후 해당 강의실 사용 가능

5. 학생용 페이지

 5-1. 회의용/자습용 중 어떤 용도로 강의실을 이용할건지 선택

 5-2. 자습용 선택 후 원하는 강의실 선택 (회의용은 교수님 페이지와 같아 따로 캡쳐하지 않았습니다.)
  1. 하단의 숫자는 총 좌석 수를 나타내며 실제 앱 구동 시 잔여좌석이 앞에 표기됩니다.
    예) 29 / 36
   
 5-3. 강의실 선택을 다시 한 번 묻는 팝업창 실행 후 완전 선택되면 해당 좌석의 색이 변경되며 신청 완료 문구가 표시됩니다.
